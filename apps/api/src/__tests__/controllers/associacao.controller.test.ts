...
describe("POST /associacoes", () => {
  beforeEach(() => vi.clearAllMocks());

  it("returns 400 when nome is missing", async () => {
    const res = await request(app).post("/associacoes").send({});
  ...
  it("returns 201 on success", async () => {
    ...
    const res = await request(app)
      .post("/associacoes")
      .send({ nome: "Associação Rural Vale Verde", cnpj: "12.345.678/0001-99" });
...
describe("GET /associacoes/:id", () => {
  beforeEach(() => vi.clearAllMocks());

  it("returns 404 when not found", async () => {
    mockGet.mockResolvedValue(null);
    const res = await request(app).get("/associacoes/bbb-222");
...
describe("PUT /associacoes/:id", () => {
  beforeEach(() => vi.clearAllMocks());

  it("returns 404 when not found", async () => {
    mockUpdate.mockResolvedValue({ error: "not_found" });
    const res = await request(app).put("/associacoes/bbb-222").send({ nome: "Novo" });
    expect(res.status).toBe(404);
  });
  ...
  it("returns 409 on CNPJ conflict during update", async () => {
    ...
    const res = await request(app)
      .put("/associacoes/bbb-222")
      .send({ cnpj: "00.000.000/0001-00" });
    expect(res.status).toBe(409);
  });
});

describe("DELETE /associacoes/:id", () => {
  beforeEach(() => vi.clearAllMocks());

  it("returns 404 when not found", async () => {
    mockDelete.mockResolvedValue({ error: "not_found" });
    const res = await request(app).delete("/associacoes/bbb-222");
    expect(res.status).toBe(404);
  });
